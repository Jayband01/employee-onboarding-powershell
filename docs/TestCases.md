...
Testing on September 17, 2026 (UTC) confirmed 40 processed records, 30 Ready for Provisioning, 10 Exceptions, and exit code 2. All exception reasons below matched, including both members of duplicate pairs. TC01-TC18 were rerun after the source-data cleanup in Windows PowerShell 5.1.26100.9444 and PowerShell 7.6.5. The script also passed direct execution in PowerShell 7.
...
Rows 35 and 36 share mateo.delgado@example.com after trimming. Rows 39 and 40 share E1079 after trimming and case-insensitive comparison (row 40 contains ` e1079 `). Both members of each pair must fail. Row 37 has two missing fields. Row 39 has a whitespace-only FirstName.

## Source-data cleanup cases

The supplied feed now includes ordinary formatting differences alongside the existing invalid records:

| SourceRow | Raw input | Expected handling |
| --- | --- | --- |
| 1 | ` Amara `; department `finance` | Trim the name; report department as Finance. |
| 2 | ` information TECHNOLOGY `; `IT Support Analyst  ` | Report Information Technology and the trimmed title; keep the title-specific group. |
| 3, 5 | Employment types `contract` and `part-time` | Match Contract and Part-Time; keep the usual access assignments. |
| 7 | Job title ` Data Analyst ` | Trim the title and include BI-Reporting. |
| 8 | ID ` E1048 ` and trailing email whitespace | Report trimmed values; remain ready. |
| 36, 40 | Spaces around a duplicate email; ID ` e1079 ` | Both duplicate pairs still fail, including the otherwise invalid ID pair. |

These changes do not add invalid employees. The measured totals remain 30 ready and 10 exceptions because the existing normalization handles them.
...
| TC18 | Formatting in the supplied HR feed | Run the rows listed in Source-data cleanup cases; compare with a trimmed, canonical-case copy | Formatting-only changes produce the same statuses and groups. Required-field and duplicate exceptions remain unchanged. |
